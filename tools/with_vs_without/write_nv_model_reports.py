...
def _write_incomplete_overview() -> None:
    ...
    lines = [
        ...
        "## Document Matrix",
        "",
        "| Skill | Detailed reports | Current status |",
        "|---|---|---|",
    ]
    ...
    lines.extend(
        [
            ...
            "## Generated Artifacts",
            "",
            "Study JSONs, large generated NIfTI volumes, checkpoints, and "
            "command outputs live under `runs/with_vs_without_nv/` and remain "
            "gitignored.",
            "",
...
def _tolerant_command_candidate(text: str) -> tuple[str | None, str]:
    ...
    if re.match(r"^\s*(?:bash|sh|shell)\s*\n", candidate, flags=re.IGNORECASE):
        ...
        return (
            (candidate, "malformed_language_prefix")
            if candidate
            else (None, "empty_after_language_prefix")
        )
...
def write_codex(skill: str) -> dict[str, Any]:
    ...
    lines.extend(
        [
            ...
            "generated command. " + CACHE_ENV_TEXT,
            "",
...
def write_overview(codex: list[dict[str, Any]], nemotron: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        f"Audit status: {_report_status_clause()}.",
        "",
        "This checked-in aggregate summarizes the corrected with-vs-without "
        "protocol. Detailed per-skill generated reports are reproducible local "
        "artifacts under `runs/with_vs_without_nv/reports/` rather than "
        "committed records:",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_snapshot(repeats=args.repeats)
    return 0

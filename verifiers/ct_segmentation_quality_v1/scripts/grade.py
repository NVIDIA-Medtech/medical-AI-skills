...
produced a geometrically-aligned, label-set-valid mask; this verifier asks
...
VERIFIER_ID = "medagent.verifiers.ct_segmentation_quality_v1"
VERIFIER_VERSION = "0.1.0"
TARGET_SKILL_IDS = {
    "nv_segment_ct",
    "medagent.nv_segment_ct",
    "nv_segment_ctmr",
    "medagent.nv_segment_ctmr",
}
CTMR_TARGET_SKILL_IDS = {"nv_segment_ctmr", "medagent.nv_segment_ctmr"}

# Maximum tolerated "extra" classes — labels present in the output that
# were not in `label_prompts_requested`. 0 is strict; >0 forgives small
# noise-level overflows. The wrapper's own sanity gate enforces strict
# matching, but the verifier accepts a small floor so an evidence pack
# from upstream-form tooling that segments 1-2 extra classes doesn't
# false-positive.
LABEL_SET_SUBSET_TOLERANCE = 0

ANATOMY_BOUNDS_PATH = Path(__file__).resolve().parent.parent / "validators" / "anatomy_bounds.json"
...
def _connected_component_stats(mask: np.ndarray) -> tuple[int, float]:
    """Return (component_count, largest_cc_fraction) for a boolean mask.

    Uses scipy.ndimage.label with 26-connectivity if available; falls back to
    a numpy-only flood fill otherwise. Returns (0, 0.0) for empty masks.
    """
    total = int(mask.sum())
    if total == 0:
        return 0, 0.0
    try:
        ...
        # Conservative fallback: report 1 CC with full coverage so we don't
        # spuriously fail packs that lack scipy. Correctness of CC counting
        # is best-effort without scipy.
        return 1, 1.0
...
def _anatomy_plausibility(
    ...
    inventory: dict[str, Any] = {
        "label_map_path": _public_path(mask_path),
    ...
    if mask_path is None or not mask_path.exists():
        ...
        plausibility["checks"].append(
            make_check("label_map_present", False, f"path={_public_path(mask_path)} not found")
        )
    ...
    checks = [
        make_check(
            "label_map_present",
            True,
            f"loaded {_public_path(mask_path)}",
        ),
        make_check(
            "any_class_present",
            len(per_class) > 0,
...
def _gt_metrics(
    ...
    if gt_path is None:
        return skipped
    skipped["ground_truth_path"] = _public_path(gt_path)
    ...
    if not gt_path.exists():
        return {
            **skipped,
            "verdict": "skipped",
            "acceptable": True,
            "reason": f"ground_truth_path {_public_path(gt_path)} not found on disk",
        }
    ...
    checks = [
        make_check(
            "gt_loaded",
            True,
            f"loaded GT from {_public_path(gt_path)}, scored {len(per_class)} classes",
        ),
        make_check(
...
def _label_set_subset(
    ...
    """Check that the produced label set is a subset of what was requested.

    A Medical AI Skills skill that respects `--label-prompts "1,3,5,14"` should
    produce a mask with foreground IDs ⊆ {1, 3, 5, 14}. An upstream tool
    invoked without `label_prompt` will produce "segment everything"
    (all 132 VISTA3D classes), which the user did not ask for. The
    anatomy_plausibility tier doesn't catch this because each extra
    class IS individually plausible — only the *requested vs delivered*
    comparison catches it.
    """
    requested_set = {int(x) for x in requested}
    present_set = {int(x) for x in label_ids_present}
    if not requested:
        # No requested set on the pack — can't compute subset relation.
        return {
            "verdict": "skipped",
...
def _target_modality_supported(skill_id: str, input_modality: Any) -> bool:
    if skill_id in CTMR_TARGET_SKILL_IDS:
        return input_modality == "CT_BODY"
    return True
...
def grade(pack_dir: Path) -> dict[str, Any]:
    ...
    input_modality = input_block.get("modality")
    modality_supported = _target_modality_supported(skill_id, input_modality)
    ...
    # the 4 requested organs" — every extra class is individually plausible.
    # Compare requested set vs delivered set explicitly.

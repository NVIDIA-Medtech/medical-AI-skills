...
def grade(pack_dir: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        ...
        make_check(
            "phi_tags_list_shape",
            isinstance(phi_tags_found, list)
            and all(isinstance(item, str) for item in phi_tags_found),
    ...
    if isinstance(phi_present, bool) and isinstance(phi_tags_found, list):
        checks.append(
            make_check(
                "phi_flag_consistent",
                (phi_present and len(phi_tags_found) > 0)
                or (not phi_present and len(phi_tags_found) == 0),

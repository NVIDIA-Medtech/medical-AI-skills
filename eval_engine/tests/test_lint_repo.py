...
def test_has_tracked_upstream_ref_accepts_commit_revision_or_version() -> None:
    assert _has_tracked_upstream_ref(
        {
            "upstream_refs": [
                {"kind": "github_repo", "name": "demo", "git_commit": "a" * 40}
            ]
        }
    )
    assert _has_tracked_upstream_ref(
        {
            "upstream_refs": [
                {"kind": "huggingface_repo", "name": "demo", "revision": "main"}
            ]
        }
    )
    assert _has_tracked_upstream_ref(
        {
            "upstream_refs": [
                {"kind": "pypi_package", "name": "demo", "version_constraint": ">=1"}
            ]
        }
    )
...
def test_mentions_runtime_upstream_patch_allows_config_staging() -> None:
    ...
    assert not _mentions_runtime_upstream_patch(
...
def test_generated_record_path_guard_flags_regenerable_records() -> None:
    ...
    assert _is_generated_record_path(
        "docs/with-vs-without-nv-generate-mr-codex-opus.md"
    )
    assert _is_generated_record_path(
        "examples/studies/with_vs_without_skill/example/with.json"
    )
    assert _is_generated_record_path(

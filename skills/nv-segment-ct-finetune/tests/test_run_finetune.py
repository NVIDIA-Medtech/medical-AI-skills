...
def test_prepare_bundle_files_stages_train_configs_from_local_upstream(tmp_path, monkeypatch):
    ...
    for name in ("train.json", "train_continual.json", "multi_gpu_train.json", "evaluate.json"):
...
def test_metric_compat_config_stack_skips_when_mean_dice_accepts_num_classes(monkeypatch):

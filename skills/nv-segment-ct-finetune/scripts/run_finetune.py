...
ANATOMY_EXPECTED_COMPONENTS = {  # solitary organs; user can override
    "spleen": 1, "liver": 1, "pancreas": 1, "stomach": 1, "gallbladder": 1,
    "right kidney": 1, "left kidney": 1,
...
def _upstream_config_dirs() -> list[Path]:
    ...
    dirs.extend(
        [
            _REPO_ROOT / ".workbench_data" / "upstreams" / "NV-Segment-CTMR" / "NV-Segment-CT" / "configs",
            _REPO_ROOT / ".workbench_data" / "upstreams" / "NV-Segment-CTMR" / "NV-Segment-CTMR" / "configs",
        ]
    )
    unique: list[Path] = []
    seen: set[Path] = set()
...
def detect_env() -> dict:
    ...
    return {
        "gpu_count": n, "gpu_name": gpu_name,
        "gpu_total_mb": total_mb, "gpu_free_mb": free_mb,
        "host_ram_mb": ram_mb, "cuda_available": n > 0,
...
def _audit_volume(img_path: Path, lab_path: Path, user_idx: int,
                  anatomy: Optional[str]) -> dict:
...
def inspect_and_build_datalist(dataset_dir: Path, output_dir: Path,
                               user_label_idx: int,
                               anatomy: Optional[str] = None) -> tuple[Path, dict]:
    ...
    for img_p in images:
        ...
        lab_p = next((dataset_dir / "labelsTr" / f"{stem}{s}"
                      for s in NIFTI_SUFFIXES
                      if (dataset_dir / "labelsTr" / f"{stem}{s}").exists()), None)
        if lab_p is None:
        ...
        pairs.append({"image": str(img_p.relative_to(dataset_dir)),
    ...
    for p in pairs[: min(int("5"), len(pairs))]:
        ...
        seen_labels.update(int(v) for v in np.unique(
            np.asarray(nib.load(str(lab_p)).dataobj)))
    ...
    audit = {
        ...
        "n_pairs": len(split_pairs), "n_folds": int("5"),
...
def audit_existing_datalist(dataset_dir: Path, datalist: Path,
                            user_label_idx: int,
                            anatomy: Optional[str] = None) -> dict:
...
def resolve_mapping(target_anatomy: Optional[str], user_idx: int,
                    literal: Optional[str]) -> tuple[dict, dict]:
    if literal:
    ...
    return ({"default": [[user_idx, d[key]]]},
            {"source": "anatomy_lookup", "anatomy": target_anatomy,
             "user_idx": user_idx, "vista3d_idx": d[key]})


# --- bundle run + log parse -------------------------------------------------

def build_override(dataset_dir: Path, datalist: Path, mapping: dict,
                   patch: list[int], cache_rate: float, epochs: int,
                   lr: float, ckpt_dir: Path, train_output_dir: Path,
                   auto_seg: bool = False) -> dict:
    ...
    override = {
        ...
        "patch_size": patch, "patch_size_valid": patch,
        "label_mappings": mapping,
        "epochs": epochs, "val_interval": 1, "val_at_start": True,
        "learning_rate": lr,
    ...
    *,
    ...
    if multi_gpu:
        cmd = ["torchrun", "--nnodes=1", f"--nproc_per_node={nproc}",
               "-m", "monai.bundle", "run", "--config_file", cfg_arg,
               "--bundle_root", str(BUNDLE_DIR)]
    else:
        cmd = [sys.executable, "-m", "monai.bundle", "run",
               "--config_file", cfg_arg, "--bundle_root", str(BUNDLE_DIR)]
    if extra_args:
    ...
    if subprocess.run(["which", "nvidia-smi"], capture_output=True).returncode == 0:
        ...
        smi = subprocess.Popen(
            smi_cmd, stdout=smi_out, stderr=subprocess.DEVNULL)
    try:
        env = os.environ.copy()
        if force_single_gpu and "CUDA_VISIBLE_DEVICES" not in env:
            env["CUDA_VISIBLE_DEVICES"] = "0"
        with open(log_path, "w") as f:
            rc = subprocess.call(cmd, cwd=BUNDLE_DIR, stdout=f,
...
def main(
    ...
    dataset_dir: Optional[Path] = typer.Option(None, "--dataset-dir",
        help="Root containing imagesTr/ and labelsTr/."),
    datalist: Optional[Path] = typer.Option(None, "--datalist",
        help="MONAI-bundle datalist JSON. Optional; auto-built when omitted."),
    target_anatomy: Optional[str] = typer.Option(None, "--target-anatomy",
        help="Anatomy name resolved against bundle/label_dict.json."),
    user_label_idx: int = typer.Option(1, "--user-label-idx",
        help="Label index that --target-anatomy occupies in the user's datalist."),
    label_mapping: Optional[str] = typer.Option(None, "--label-mapping",
    ...
    patch_size: Optional[str] = typer.Option(None, "--patch-size",
        help='JSON list. Overrides auto-derived patch size.'),
    ...
    auto_seg: bool = typer.Option(False, "--auto-seg",
        help="Use automatic class-prompt training: drop_label_prob=0.0, drop_point_prob=1.0."),
    skip_formal_eval: bool = typer.Option(False, "--skip-formal-eval",
    ...
    if dataset_dir is None:
        raise typer.BadParameter(
            "--dataset-dir required (or use --sanity / --smoke).")
    if not dataset_dir.is_dir():
    ...
    if datalist is None:
        datalist, dataset_audit = inspect_and_build_datalist(
            dataset_dir, output_dir, user_label_idx=user_label_idx,
        ...
        if smoke:
            ...
            dataset_audit = audit_existing_datalist(
                dataset_dir, datalist, user_label_idx=user_label_idx,
                anatomy=target_anatomy)
    ...
    plan_cache = cache_rate if cache_rate is not None else pick_cache_rate(n_train, env["host_ram_mb"])
    plan_epochs = epochs if epochs is not None else (2 if smoke else int("5") if sanity else int("50"))
    ...
    plan = {
        "patch_size": plan_patch, "train_dataset_cache_rate": plan_cache,
        "epochs": plan_epochs, "learning_rate": learning_rate,
        "nproc_per_node": nproc, "multi_gpu": multi_gpu,
        ...
        "rationale": [
            ...
            "single-gpu DFW Task06 recipe" if sanity else
            "single-gpu smoke preset" if smoke else "host GPU policy",
        ],
    }

    override = build_override(dataset_dir, datalist, mapping,
                              plan_patch, plan_cache, plan_epochs, learning_rate,
                              ckpt_dir, train_output_dir,
                              auto_seg=auto_seg)
    override_file = (
        write_config("train_continual_task06_lung.json", override)
        if sanity else write_config("auto_override.json", override)
    )
    ...
    checkpoint_comparisons = {
        "best": (
            ...
            if finetune_ckpt.exists() else None
        ),
    }
    ...
    result = {
        ...
        "version": VERSION,
        "input": {
            "dataset_dir": str(dataset_dir), "datalist": str(datalist),
            "n_train_cases": n_train,
            "label_mappings": mapping, "label_mapping_resolution": mapping_src,
            "dataset_audit": dataset_audit, "smoke": smoke,
            "sanity": sanity, "auto_seg": auto_seg, "formal_eval": formal_eval,
        },
        "environment": env,
        "plan": plan,
        "invocation": {
            ...
            "commands": {
                "eval_pretrained": (
                    ...
                    if formal_pre_cmd else None
                ),
                "finetune": " ".join(shlex.quote(c) for c in cmd),
                "eval_finetuned": (
                    " ".join(shlex.quote(c) for c in formal_post_cmd)
                    if formal_post_cmd else None
                ),
            },
            "command_prefix": " ".join(cmd[: cmd.index("run") + 1])
                              if "run" in cmd else cmd[0],
        ...
        "output": {
            ...
            "finetuned_ckpt_matches_pretrained_weights": (
                ...
                if checkpoint_comparisons["best"] is not None else None
            ),

...
- Manifest inputs include the dataset and preset controls, `skip_formal_eval`, plus optional `mlflow_tracking_uri`, `mlflow_experiment_name`, and `mlflow_run_name` tracking controls.
...
python skills/nv-segment-ct-finetune/scripts/run_finetune.py \
  ...
  --mlflow-experiment-name /Shared/nvseg-task06-demo
```

Setting `--mlflow-experiment-name` enables MONAI's documented
`--tracking mlflow` path. It streams training loss and validation metrics using
MONAI's built-in rank-zero handlers. Omit `--mlflow-tracking-uri` to use a local
file store under `<output-dir>/mlruns`, or pass `databricks` for the caller's
configured Databricks workspace. The wrapper disables executed-config uploads
and does not override patch size, transforms, optimizer values, DataLoader

...
def main(
    ...
    filter_substring: str = typer.Option(
        None, "--filter", "-f", help="Show only classes whose name contains this substring (case-insensitive).",
    ),
    ...
    if controllable:
        typer.echo("# Controllable anatomies (accept controllable_anatomy_size = [[name, scale], ...])")
    ...
    if region is not None and region not in grouped:
        typer.echo(
            f"error: region {region!r} not recognized. "
            f"Choices: {list(grouped.keys())}",
            err=True,
        )
        raise typer.Exit(2)
    ...
    typer.echo(
        f"# Supported body_region values for synthesis: {list(SUPPORTED_BODY_REGIONS)}"
    )

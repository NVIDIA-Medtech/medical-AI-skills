#!/usr/bin/env python3
"""Wrapper for NV-Generate-CTMR MR-brain diffusion-UNet finetuning.

The wrapper validates a user datalist, stages the upstream JSON configs, then
calls the existing NV-Generate-CTMR script entrypoints. It does not execute the
training notebook and it does not reimplement embedding extraction, diffusion
...
def _network_config_name(version: str) -> str:
    return "config_network_ddpm.json" if version == "ddpm-ct" else "config_network_rflow.json"


def _config_paths(upstream_root: Path, version: str) -> tuple[Path, Path, Path]:
    return (
        upstream_root / "configs" / _network_config_name(version),
        upstream_root / "configs" / f"environment_maisi_diff_model_{version}.json",
        upstream_root / "configs" / f"config_maisi_diff_model_{version}.json",
    )
...
def _stage_configs(args: argparse.Namespace, upstream_root: Path) -> dict[str, Any]:
    model_def_src, env_src, model_src = _config_paths(upstream_root, VERSION)
    ...
    env_config["modality_mapping_path"] = str((upstream_root / "configs" / "modality_mapping.json").resolve())
    ...
    return {
        ...
        "input": {
            **dataset,
            ...
            "infer_spacing": args.infer_spacing,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--infer-dim", type=lambda s: _parse_triplet(s, int), default=[256, 256, 256])
    parser.add_argument("--infer-spacing", type=lambda s: _parse_triplet(s, float), default=[1.0, 1.0, 1.0])
    ...
    parser.add_argument("--output-prefix", default="mr_brain_finetune")

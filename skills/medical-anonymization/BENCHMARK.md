...
## With-skill vs without-skill (medical-anonymization)

Local PHI-safe harness comparing `medical-anonymization/SKILL.md` against an
upstream NeMo README baseline on the same staged inputs:

| Task | With skill | Without skill | Expected without gap |
|------|----------:|-------------:|----------------------|
| DICOM metadata (85 studies) | 3.2s | 1.6s | `no_structured_policy_applied` — PHI columns unchanged |
| EHR CSV (2 rows, NeMo preview) | 16.9s | 23.7s | `structured_id_not_hashed` — MRN left verbatim |
| Reports CSV (2-row NeMo preview) | 32.9s | 43.8s | text redacted only; no KV/audit |

**Totals (2026-07-11, local GLiNER + Ollama medgemma:27b):** with 53.1s / without 69.0s; **3/3 passes each arm** (without arm allows expected failure modes above).

Full rad-style report (LLM judge + sign test + HTML dashboard):

- [`docs/anonymization-with-vs-without-medical-local.md`](../../docs/anonymization-with-vs-without-medical-local.md)
- [`docs/anonymization-with-vs-without-medical-local.html`](../../docs/anonymization-with-vs-without-medical-local.html)

Regenerate::

```bash
/home/marc/.conda/envs/reports-openai/bin/python \
  skills/medical-anonymization/scripts/generate_with_vs_without_judge_report.py
```

Capability-only harness (DICOM/EHR/reports timing, no judge):

```bash
/home/marc/.conda/envs/reports-openai/bin/python \
  skills/medical-anonymization/scripts/run_with_vs_without_eval.py \
  --output-dir /tmp/medical_anon_with_vs_without_check \
  --report-num-records 2
```

See [`docs/with-vs-without-results.md`](docs/with-vs-without-results.md) · protocol: [`docs/with-vs-without-experiment.md`](docs/with-vs-without-experiment.md)

## With-skill vs without-skill (report-anonymization, remote)

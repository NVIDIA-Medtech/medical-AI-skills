...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Nemotron | with | 3 | 3 | 3 | 6,714 | 4,294 | 0 | 11,008 | 3,669.3 | 3 | 88.1 |
| Nemotron | without | 3 | 0 | 3 | 9,666 | 13,963 | 0 | 23,629 | 7,876.3 | 0 | n/a |
...
export NV_GENERATE_ROOT="${NV_GENERATE_ROOT:-.workbench_data/upstreams/NV-Generate-CTMR}" && \
python -m pip install -r "$NV_GENERATE_ROOT/requirements.txt" && \
python skills/nv-generate-mr-brain/scripts/run_mr_brain.py runs/with_vs_without_nv/_inputs/nv_generate_mr_brain/request.json --output-dir runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/with/repeat_2 --modality mri_t1 --random-seed 42
```
...
python skills/nv-generate-mr-brain/scripts/run_mr_brain.py \
  ...
  --output-dir runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/with/repeat_3 \
  --modality mri_t1 \
  --random-seed 1234
```
...
```bash
mkdir -p runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_1 && cd runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_1 && cp -r ../../../configs . && sed -i 's/"output_dir":.*/"output_dir": ".",/' configs/environment_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"dim":.*/"dim": [256, 256, 256],/' configs/config_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"spacing":.*/"spacing": [1.0, 1.0, 1.0],/' configs/config_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"modality":.*/"modality": 9,/' configs/config_maisi_diff_model_rflow-mr-brain.json && network="rflow" && generate_version="rflow-mr-brain" && python -m scripts.download_model_data --version ${generate_version} --root_dir "./" --model_only && python -m scripts.diff_model_infer -t ./configs/config_network_${network}.json -e ./configs/environment_maisi_diff_model_${generate_version}.json -c ./configs/config_maisi_diff_model_${generate_version}.json
```
...
python3 -c "
import json, sys
path = 'runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json'
with open(path, 'r') as f:
    data = json.load(f)
data['diffusion_unet_inference']['dim'] = [256, 256, 256]
data['diffusion_unet_inference']['spacing'] = [1.0, 1.0, 1.0]
data['modality'] = 9
data['random_seed'] = 0
with open(path, 'w') as f:
    json.dump(data, f, indent=2)
" && \
python -m scripts.download_model_data --version rflow-mr-brain --root_dir runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3 --model_only && \
python -m scripts.diff_model_infer -t runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_network_rflow.json -e runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/environment_maisi_diff_model_rflow-mr-brain.json -c runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json
```
...
| Source | Path |
|---|---|
| Study JSON and comparison | `runs/with_vs_without_nv/studies/nv_generate_mr_brain_nemotron_correction/` |

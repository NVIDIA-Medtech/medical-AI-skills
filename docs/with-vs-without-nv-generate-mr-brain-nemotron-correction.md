# `nv_generate_mr_brain`: Nemotron LLM+SKILL.md vs LLM+README Baseline Study
...
> The image-generation request is at runs/with_vs_without_nv/_inputs/nv_generate_mr_brain/request.json. Generate one T1 brain MR image and write generated NIfTI volumes under runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/with/repeat_1.
...
|---|---|---:|---:|---|---|---|---|
| Nemotron | with | 5.0/5 | 3/3 | mean 0.0; unresolved 0; values [0, 0, 0] | 0 (3) | shape=(256, 256, 256) (3) | none |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Nemotron | with | 3 | 3 | 3 | 6,414 | 5,499 | 0 | 11,913 | 3,971.0 | 3 | 51.1 |
| Nemotron | without | 3 | 0 | 3 | 9,660 | 17,389 | 0 | 27,049 | 9,016.3 | 0 | n/a |
...
python skills/nv-generate-mr-brain/scripts/run_mr_brain.py runs/with_vs_without_nv/_inputs/nv_generate_mr_brain/request.json --output-dir runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/with/repeat_1 --modality mri_t1 --random-seed 1234
```
...
python skills/nv-generate-mr-brain/scripts/run_mr_brain.py \
  runs/with_vs_without_nv/_inputs/nv_generate_mr_brain/request.json \
  --output-dir runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/with/repeat_2 \
  --modality mri_t1 \
  --random-seed 1234
```
...
```bash
OUTPUT_DIR="runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_2" && mkdir -p "$OUTPUT_DIR" && cd "$OUTPUT_DIR" && python -m scripts.download_model_data --version rflow-mr-brain --root_dir "." --model_only && cp ../configs/config_network_rflow.json . && cp ../configs/environment_maisi_diff_model_rflow-mr-brain.json . && cp ../configs/config_maisi_diff_model_rflow-mr-brain.json . && sed -i 's/"dim": \[[^]]*\]/"dim": [256,256,256]/' config_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"spacing": \[[^]]*\]/"spacing": [1.0,1.0,1.0]/' config_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"modality": [0-9]\+/"modality": 9/' config_maisi_diff_model_rflow-mr-brain.json && sed -i 's/"output_dir": "[^"]*"/"output_dir": "."/' environment_maisi_diff_model_rflow-mr-brain.json && python -m scripts.diff_model_infer -t config_network_rflow.json -e environment_maisi_diff_model_rflow-mr-brain.json -c config_maisi_diff_model_rflow-mr-brain.json
```

Repeat 3: score 3/5, passed=no, steps=unresolved, exit=None

```bash
mkdir -p runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3 && \
cp configs/config_network_rflow.json runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/ && \
cp configs/environment_maisi_diff_model_rflow-mr-brain.json runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/ && \
cp configs/config_maisi_diff_model_rflow-mr-brain.json runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/ && \
sed -i 's|"output_dir":.*|"output_dir": "runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3",|' runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/environment_maisi_diff_model_rflow-mr-brain.json && \
sed -i 's/"dim": [0-9]*, [0-9]*, [0-9]*/"dim": [256, 256, 256]/' runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json && \
sed -i 's/"spacing": [0-9]*\.[0-9]*, [0-9]*\.[0-9]*, [0-9]*\.[0-9]*/"spacing": [1.0, 1.0, 1.0]/' runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json && \
sed -i 's/"modality": [0-9]*/"modality": 9/' runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json && \
python -m scripts.download_model_data --version rflow-mr-brain --root_dir ./ --model_only && \
python -m scripts.diff_model_infer \
    -t runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_network_rflow.json \
    -e runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/environment_maisi_diff_model_rflow-mr-brain.json \
    -c runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/without/repeat_3/config_maisi_diff_model_rflow-mr-brain.json
```
...
| Source | Path |
|---|---|
| Study JSON and comparison | `examples/studies/with_vs_without_skill/nv_generate_mr_brain_nemotron_correction/` |
| Generated outputs | `runs/with_vs_without_nv/nv_generate_mr_brain_nemotron_correction/` |
| Fair path-prompt artifact | `tools/nat_audit/data/eval_nv_model_studies_nv_generate_mr_brain_prompts.json` |

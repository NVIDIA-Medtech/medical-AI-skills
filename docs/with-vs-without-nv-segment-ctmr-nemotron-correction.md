...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Nemotron | with | 3 | 3 | 3 | 7,488 | 4,063 | 0 | 11,551 | 3,850.3 | 3 | 56.1 |
| Nemotron | without | 3 | 0 | 3 | 12,054 | 19,798 | 0 | 31,852 | 10,617.3 | 0 | n/a |
...
python skills/nv-segment-ctmr/scripts/run_ctmr.py runs/with_vs_without_nv/_inputs/nv_segment_ctmr/input.nii.gz \
  --modality CT_BODY \
  --output-dir runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/with/repeat_3
```
...
```bash
mkdir -p runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_2 && python -m monai.bundle run --config_file="['configs/inference.json', 'configs/batch_inference.json']" --input_dir="runs/with_vs_without_nv/_inputs/nv_segment_ctmr" --output_dir="runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_2" --modality CT_BODY --separate_folder false
```
...
mkdir -p runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_3 &&
python -m monai.bundle run --config_file="['configs/inference.json', 'configs/batch_inference.json']" \
    --input_dir=runs/with_vs_without_nv/_inputs/nv_segment_ctmr \
    --output_dir=runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_3 \
    --modality CT_BODY &&
mv runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_3/input/input_trans.nii.gz \
   runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_3/ &&
rmdir runs/with_vs_without_nv/nv_segment_ctmr_nemotron_correction/without/repeat_3/input
```
...
| Source | Path |
|---|---|
| Study JSON and comparison | `runs/with_vs_without_nv/studies/nv_segment_ctmr_nemotron_correction/` |
